	...
	public class RandomMap extends Sprite {
		...
		private function addBackground():void {
			for (var k:int = 0; k < 50; k++) {
				for (var l:int = 0; l < 50; l++) {
					...
					CameraManager._staticLayer.addChildAt(underground, 0);
				}
			}
		}
		...
		private function setupLaunchPoint():void {
			...
			for each (var point:Point in possiblePoints) {
				...
				var key:CircleStaticActor = new CircleStaticActor(CameraManager._camera, location, 12, CircleStaticActor.KEY);
		...
		public function addCheckpoint():void {
			...
			var location:Point = new Point((point.x - 15) / 30 + 1, (point.y - 15) / 30 + 1);
			var checkpoint:CircleStaticActor = new CircleStaticActor(CameraManager._camera, location, 12, CircleStaticActor.CHECKPOINT);
		...
		private function createBox2dObjects():void {
			...
			var newBox:ArbiStaticActor = new ArbiStaticActor(CameraManager._camera, location, box);*/
			for each (var k:Array in theCave) {
				...
				for each (var r:int in k) {
					...
					if (r == 0) {
						...
						var box:Array = [new Point(l.x, l.y), 
										 new Point(l.x + tileSize, l.y), 
										 new Point(l.x + tileSize, l.y + tileSize), 
										 new Point(l.x, l.y + tileSize)];
						boxArray.push(box);
				...
				var newBox:ArbiStaticActor = new ArbiStaticActor(CameraManager._camera, new Point(offsetX, offsetY), boxArray);
				i++;
			}
		...
		private function remakeInRectangles():void {
			...
			for (m = 0; m < mapHeight; m++) {
				for (n = 0; n < mapWidth; n++) {
					if (theCaveCopy[n][m] == 0) {
						...
						Fichez.makeRect(3 * (a - n), 3 * (b - m), 3 * n, 3 * m);
			...
			Raster.rasterizeAll();
			//LevelDirector.rasterize2(CameraManager._cameraStaticLayer);
			CameraManager._camera.ZOOM = 10;
			/*Raster.cacheAnimation("wmr");
