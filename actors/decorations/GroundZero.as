package {
	...
	* @author Mad Mike
	*/
	public class GroundZero extends Platform {
		private var body:b2Body;
		private var costume:MovieClip;
		public var width:Number = 1;
		public var height:Number = 1;
		
		public function GroundZero(x:Number, y:Number, w:Number = 1, h:Number = 1) {
			super();
		...
		override protected function createCostumes():void {
			...
			var bitmapData:BitmapData = new BitmapData(width * 20 * scaleRate, height * 20 * scaleRate, true, 0x00000000);
			var u:MovieClip = new ug();
			for (var j:int = 0; j < height * 20 / 43 + 1; j++) {
				for (var i:int = 0; i < width * 20 / 92 + 1; i++) {
					var m:Matrix = new Matrix();
					m.translate( -10 * scaleRate + i * 92, j * 43);
